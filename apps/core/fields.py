...
def clean_html(raw_html):
    """
    HTML-очистка с помощью BeautifulSoup.

    :param raw_html: исходный HTML-код
    :return: очищенный текст
    """
    # создаем объект BeautifulSoup из HTML-кода
    soup = BeautifulSoup(raw_html, "lxml")
    # получаем текст из объекта
    cleantext = soup.text
    return cleantext
    ...
    def _get_populate_value(self, instance):
        ...
        if callable(self.populate_from):
            ...
            return "-".join(
                ...
                for fieldname in self.populate_from
            )
        else:
    ...
    def _make_unique(self, value, instance):
        ...
        if self.unique:
            if qs.filter((self.attname, slug)).exists():
                ...
                slugs = qs.filter(**lookup).values_list(
